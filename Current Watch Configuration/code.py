...
def display_time(ct):
    ...
    if alarm_set:
        display.circle(4, 4, 4, 1)
        display.line(4, 4, 6, 6, 1)
        display.line(4, 4, 4, 6, 1)
        if ct.tm_hour == alarm_hour and ct.tm_min == alarm_min:
            state.pop()
            state.append(flash_alarm)
    year_len = len(f"{ct.tm_year}")
    display.text(f"{ct.tm_year}", (display.width // 2 - year_len) - 8, 16, 1, size=1)
    day_len = len(f"{days[ct.tm_wday]}")
    display.text(f"{days[ct.tm_wday]}", (display.width // 2 - day_len * 5) - 6, 0, 1, size=2)
...
def alarm(hour, min, set, ct):
    ...
    if button1.value:
        display.fill(0)
        display.text("ALARM SET", 0, 32, 1, size=2)
        display.show()
        state.pop()
        time.sleep(1)
        set = True
